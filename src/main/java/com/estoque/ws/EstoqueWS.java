...
public class EstoqueWS {
    ...
    // Annotation para redefinir o nome do elemento que embrulha a mensagem na Resposta
    @WebResult(name = "item")
    // Com o @ResponseWrapper cada item deverá ser representado pela tag <item>, já que o nome do elemento pai será <itens>
    @RequestWrapper(localName = "listaItens")
    // Annotation para redefinir o nome do elemento que embrulha a mensagem na Requisição
    ...
    @WebResult(name = "item")
    // o throws AutorizacaoException abaixo deixa explícito que pode acontecer uma AutorizacaoException
    public Item cadastrarItem(@WebParam(name = "tokenUsuario", header = true) TokenUsuario token, @WebParam(name = "item") Item item) throws AutorizacaoException { // header = true adiciona o atributo ao header
        System.out.println("Cadastrando " + item + ", " + token);

        boolean valido = new TokenDao().isValido(token);

        if (!valido) {
            throw new AutorizacaoException("Token invalido"); // lanca exception se o token for invalido
        }
